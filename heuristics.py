'''
Heuristic Algorithms for solving cubic multidimensional knapsack problems
'''
...
'''
Apply Glover's constructive Greedy heuristic to a multi-dimension cubic knapsack

weight_option : string, optional
	specifies method for determining item weight in multi-dimensional situation. Options are
	'avg' (default) - use average weight across dimensions
	'high' - use highest weight (relative to dimenison capacity)
'''
def naive_greedy(cubic, weight_option="high"):
	n = cubic.n
	c = cubic.c
	C = cubic.C
	D = cubic.D
	...
	if weight_option == "avg":
		# use average weight across dimenions
		for i in range(n):
			total_weight = sum(a[i])
			weights[i] = total_weight/constraints
	elif weight_option == "high":
		# use max weight by percent across dimensions
		for i in range(n):
			for d in range(constraints):
				weight_percent = a[i][d]/RHS[d]
				if weight_percent > weights[i]:
					weights[i] = weight_percent

	# compute intial value ratios
	value_ratios = np.zeros(n)
	for i in N0:
		value_ratios[i] = c[i] / weights[i]
	...
	while all(RHS) > 0 and len(N0) > 0:
		...
		assert value_ratios[take_index] >= 0, "taking an item with negative value"
		...
		for i in N0:
			...
			value_ratios[i] += (C[i, take_index] + C[take_index, i])/weights[i]

			# add in new cubic contributions
			for j in N1:
				value_ratios[i] += getDValue(i, j, take_index, D)/weights[i]
...
'''
def naive_greedy_probe(cubic, weight_option="high", probe_size=5):
	n = cubic.n
	c = cubic.c
	C = cubic.C
	D = cubic.D
	...
	# repeatedly pick the best item until no more items can fit
	while all(RHS) > 0 and len(N0) > 0:
		# get the k best candidate indices
		if len(N0) >= probe_size:
			# get indices of k best value ratios
			candidates = np.argpartition(value_ratios, -probe_size)[-probe_size:]
			combo_evals = {}
			best_eval = -1
			best_combo = (0,1,2) # use this pairing by default if no combos can actually fit
			...
			for i in range(len(candidates)):
				for j in range(i+1, len(candidates)):
					for k in range(j+1, len(candidates)):
						...
						fits = True
						for d in range(constraints):
							total_weight = a[i][d] + a[j][d] + a[k][d]
							if total_weight > RHS[d]:
								fits = False
								break
						if not fits:
							continue
						...
						combination_profit += value_ratios[i]*weights[i] # CP_i
						combination_profit += value_ratios[j]*weights[j] # CP_j
						combination_profit += value_ratios[k]*weights[k] # CP_k
						...
						combination_profit = combination_profit / (weights[i]+weights[j]+weights[k])
...
def main():
	n = 40
	cdmkp = CMDKP(n=n, density=70, constraints=3,seed=0)
	...
	indices = naive_greedy_probe(cdmkp, weight_option="high")
